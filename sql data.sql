...
SELECT performance_level, COUNT(DISTINCT id_student) AS total_students, ROUND(AVG(avg_score), 2) AS average_score FROM ucation_dataset GROUP BY performance_level ORDER BY average_score DESC; 
...
SELECT engagement_level, COUNT(DISTINCT id_student) AS total_students, COUNT(DISTINCT CASE WHEN dropout_flag = 1 THEN id_student END) AS dropout_students, ROUND(100.0 * COUNT(DISTINCT CASE WHEN dropout_flag = 1 THEN id_student END) / COUNT(DISTINCT id_student), 2) AS dropout_rate FROM online_education_dataset GROUP BY engagement_level ORDER BY dropout_rate DESC;
